...
# to set up a new key
#+BEGIN_SRC sh
ssh-keygen -t rsa -f vmkey -C "ben@benswift.me" && sudo apt-get install xclip && xclip -sel clip < ~/.ssh/id_rsa.pub
#+END_SRC
...
#+BEGIN_SRC sh
sudo add-apt-repository ppa:unit193/sourcecodepro && sudo apt-get update && sudo apt-get install fonts-source-code-pro-otf
#+END_SRC
