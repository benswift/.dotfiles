...
If CRLF appears, it's a helix config/bug issue. If LF is preserved, the original
file likely had mixed/CRLF content.
...
**Conclusion**: Helix correctly respects `default-line-ending = "native"` on
macOS and preserves LF endings. The original `ledger.csv` issue was caused by
the file already having CRLF/mixed line endings before editing, not by helix
introducing them.
