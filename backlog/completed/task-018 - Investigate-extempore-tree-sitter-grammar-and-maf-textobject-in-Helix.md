...
defined in `helix/languages.toml`, but the compiled grammar may not be built on
this machine.

Steps to investigate:

- Check if `hx --grammar fetch && hx --grammar build` has been run and whether
  the extempore grammar compiled successfully
- Verify the textobjects.scm query matches correctly once the grammar is
  available
- Also check whether `Ctrl-Enter` (C-ret) works correctly for
